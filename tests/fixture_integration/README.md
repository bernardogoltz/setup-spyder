...
uv run integration --keep        # não apaga nada no fim
```

O `pyproject.toml`, o `uv.lock`, o `main.py`, o `.venv/` e o `.spyproject/`
daqui são gerados pela rotina e apagados no fim dela (a menos que você passe
`--keep`). Ficam fora do git de qualquer jeito; só este `README.md` é
versionado.
