...
`--reset-profile`) pulam enquanto o argparse ainda nao os conhece, e viram
falha com ``SETUP_SPYDER_STRICT=1``.

Nota de leitura: o plano descreve `--profile {ephemeral,project}`; a CLI
tambem tem a flag booleana `--ephemeral`, equivalente a `--profile ephemeral`
(o padrao e o perfil por projeto, em `<raiz>/.spyproject/setup-spyder/`). Os
dois conjuntos convivem aqui de proposito - o antigo tem de continuar
funcionando depois que o novo chegou.
"""
...
def test_flags_booleanas_continuam_existindo(setup_spyder_cli, flag, atributo):
...
@pytest.mark.phase0
def test_conf_dir_e_aceito(setup_spyder_cli, tmp_path):
    """Secao 5.1: "--conf-dir tem precedencia explicita sobre os dois modos"."""
    args = parse(setup_spyder_cli, ["--conf-dir", str(tmp_path)])
...
def test_hide_e_show_sao_repetiveis(setup_spyder_cli):
    args = parse(
        setup_spyder_cli, ["--hide", "a,b", "--hide", "c", "--show", ".github"]
    )
...
@pytest.mark.phase0
def test_main_repassa_todas_as_opcoes_para_launch(setup_spyder_cli, monkeypatch,
                                                  tmp_path):
    """Nenhuma opcao pode ficar orfa entre `parse_args` e `launch`."""
    recebidos = {}
    monkeypatch.setattr(
        setup_spyder_cli,
        "launch",
...
@pytest.mark.phase2
def test_main_repassa_as_opcoes_novas_para_launch(setup_spyder_cli, monkeypatch):
    recebidos = {}
    monkeypatch.setattr(
        setup_spyder_cli,
        "launch",
...
@pytest.mark.phase2
def test_main_sem_opcoes_novas_deixa_launch_decidir(setup_spyder_cli, monkeypatch):
...
@pytest.mark.phase2
def test_o_cli_repassa_tudo_ao_launcher(setup_spyder_cli, monkeypatch, tmp_path):
    """`cli.launch` so imprime o banner e delega; nenhuma opcao fica pelo caminho."""
    recebidos = {}
    monkeypatch.setattr(
        setup_spyder_cli,
        "_launch",
...
@pytest.mark.phase2
def test_reset_profile_existe(setup_spyder_cli):
    args = parse_or_pending(
        setup_spyder_cli, ["--reset-profile"], "--reset-profile"
    )
    assert args.reset_profile is True


@pytest.mark.phase2
def test_a_flag_ephemeral_atual_continua_valendo_com_profile(setup_spyder_cli):
