...
#: script instead of calling the ``spyder`` executable directly.
LAUNCHER_TEMPLATE = '''\
...
ProxyModel.PATHS_TO_HIDE = sorted(set(ProxyModel.PATHS_TO_HIDE) | set(HIDDEN))
# PATHS_TO_SHOW wins over PATHS_TO_HIDE, so drop whatever we are hiding.
ProxyModel.PATHS_TO_SHOW = [
    path for path in ProxyModel.PATHS_TO_SHOW if path not in HIDDEN
]
